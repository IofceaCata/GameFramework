...
CPlayer::CPlayer(const BackBuffer *pBackBuffer)
{
	...
	m_laserSprite = new LaserSprite("data/laser.bmp", RGB(0xff, 0x00, 0xff));
...
void CPlayer::Move(ULONG ulDirection)
{
 ...
 ///////////////////////////////////////////////////
	...
	if (ulDirection & CPlayer::DIR_RIGHT) 
		m_pSprite->mVelocity.x += 2.1;
	if (m_pSprite->mPosition.x + m_pSprite->width()/ 2 >= width)
	{
		//m_pSprite->update(0);
		//m_pSprite->mPosition.x = width ;
		//m_eSpeedState = SPEED_START;
		m_pSprite->mVelocity.x = 0;
...
////////////////////////////////////////////////////	
	if (m_pSprite->mPosition.y > height)
	{
		//m_pSprite->update(0);
		m_pSprite->mPosition.y = height;
		m_eSpeedState = SPEED_START;
		m_pSprite->mVelocity.y = 0;
...
void CPlayer::Move(ULONG ulDirection)
{
	...
	if (ulDirection & CPlayer::DIR_FORWARD) {
		if (m_pSprite->mVelocity.y > 0)
			m_pSprite->mVelocity.y -= brk;
		else if (m_pSprite->mVelocity.y >= -topSpeed)
			m_pSprite->mVelocity.y -= acc / 2;
	}
	...
	if (ulDirection & CPlayer::DIR_LEFT) {
		if (m_pSprite->mVelocity.x > 0)
			m_pSprite->mVelocity.x -= brk;
		else if (m_pSprite->mVelocity.x >= -topSpeed)
			m_pSprite->mVelocity.x -= acc;
	}
...
bool CPlayer::AdvanceLaser()
{
	if (m_blaser)
	{
		m_pExplosionSprite->mPosition = Vec2(100, 100);
		m_laserSprite->draw();
	}
	/*
	{
