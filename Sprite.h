...
{
public:
	LaserSprite(int imageID, int maskID);
	LaserSprite(const char *szImageFile, const char *szMaskFile);
	LaserSprite(const char *szImageFile, COLORREF crTransparentColor);
...
private:
	...
	// this class is not designed to be copied because it
	// is not efficient--copying bitmaps is slow (lots of memory).
	LaserSprite(const LaserSprite& rhs);
	LaserSprite& operator=(const LaserSprite& rhs);
